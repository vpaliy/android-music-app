...
class MusicPlaybackService : MediaBrowserServiceCompat(), PlaybackManager.PlaybackServiceCallback {
  ...
  init {
    App.playbackComponent?.inject(service = this)
  }

  override fun onCreate() {
    super.onCreate()
    mediaSession = MediaSessionCompat(applicationContext, TAG)
    notification = TrackNotification(service = this)
    ...
    mediaSession.setSessionActivity(PendingIntent.getActivity(context, 99, intent, PendingIntent.FLAG_UPDATE_CURRENT))
    playbackManager.updatePlaybackState(PlaybackStateCompat.STATE_NONE)
  }

  override fun onStartCommand(startIntent: Intent?, flags: Int, startId: Int): Int {
    startIntent?.action?.let {
      when (it) {
